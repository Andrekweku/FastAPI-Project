...
@pytest.mark.parametrize("title, content, published", [
    ...
    ("Andre will be great", "God willing", True ),
])
...
def test_create_post_default_publised_true(authorized_client, test_posts, test_user):
    res = authorized_client.post("/posts/", json={"title": "test title", "content": "test content"})

    created_posts = schemas.Post(**res.json())
    assert res.status_code == 201
    assert created_posts.title == "test title"
    assert created_posts.content == "test content"
